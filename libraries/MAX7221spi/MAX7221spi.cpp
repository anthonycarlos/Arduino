...
// from the datasheet DIG0 is at address 1 but is the MSB digit
const unsigned char digits[] = {8, 7, 6, 5, 4, 3, 2, 1}; // digit number address
...
void MAX7221spi::initialize(unsigned char _no_of_digits)
{
  ...
  SPI.setDataMode(SPI_MODE0);
	...
	pinMode(LOAD, OUTPUT); 
	digitalWrite(LOAD, LOW); // set load LOW for the MAX7219/7221 to receive
...
void MAX7221spi::print(float fnum, unsigned char precision) // transmit a float value
{
	...
	long fnumUpperValue = fnum;
	
	unsigned int fnumLowerValue = (fnum * (float)multiplier) - (fnumUpperValue * multiplier);
	
	unsigned long num = fnumLowerValue;
	...
	for (digit = 0; digit < no_of_digits; digit++)
	{
		...
		if (num == 0 && digit > precision)
		{			 
			if (negative && negativeFlag == false)
			{
		...
		if (digit == precision && precision != 0)
				value |= 128; 
			
		transmit(digits[digit], value);
	}
}
